...
function OrgDetail() {
    ...
    async function loadOrgDetail(id) {
        await axiosInstance.get('/job/company/jobdetail/'+id+'/').then(async (res)=>{
            setOrgDetailJob(res.data)
        }).catch((err)=>{
            router.push('/job-listing')
        })

        await axiosInstance.get('/job/company/detail/'+id+'/').then(async (res)=>{
            setOrgDetail(res.data)
        }).catch((err)=>{
            router.push('/job-listing')
        })

        await axiosInstance.get('/job/company/gallery/'+id+'/').then(async (res)=>{
            setOrgDetailGallery(res.data)
        }).catch((err)=>{
            router.push('/job-listing')
        })

        await axiosInstance.get('/job/company/link/'+id+'/').then(async (res)=>{
        ...
        {orgDetail.map((data, i) => (
            <main className="py-8" key={i}>
                <div className="container mb-12">
                    <button type="button" className="rounded-full bg-black text-white p-4 mr-4 w-[25px] h-[25px] flex items-center justify-center" onClick={(e)=>goback()}>
                                ...
                                <div className="flex items-center flex-wrap justify-between">
                                    ...
                                    <div className="w-full sm:max-w-[48%] mt-3">
                                        <h4 className="bg-white shadow-normal shadow-white rounded-[12px] py-2 px-4 text-sm flex items-center justify-center">
                                            ...
                                            {data.organisation_status ? data.organisation_status : <>N/A</>}
                                        </h4>
                                    </div>
                                    <div className="w-full sm:max-w-[48%] mt-3">
                                        <h4 className="bg-white shadow-normal shadow-white rounded-[12px] py-2 px-4 text-sm flex items-center justify-center">
                                            <i className="fa-solid fa-users mr-2"></i>
                                            {data.company_strength ? data.company_strength : <>N/A</>}
                                        </h4>
                                    </div>
                                    <div className="w-full sm:max-w-[48%] mt-3">
                                        <h4 className="bg-white shadow-normal shadow-white rounded-[12px] py-2 px-4 text-sm flex items-center justify-center">
                        ...
                        <div className="py-6 px-4">
                            <div className="flex flex-wrap items-center">
                                <div className="w-full md:max-w-[50%] py-8 md:px-8">
                                    <h3 className="text-xl mb-3">Contacts for {data.user.company_name}</h3>
                                ...
                                <div className="w-full md:max-w-[50%] py-8 md:px-8 border-t md:border-t-0 md:border-l">
                                    <div className="bg-gray-100 rounded-[25px] py-6 px-[16%]">
                                        ...
                                        }} className="border border-[#6D27F9] rounded-full py-2 px-8 text-sm text-[#6D27F9] hover:bg-gradient-to-r hover:from-[#A382E5] hover:to-[#60C3E2] hover:text-white">Send Email <i className="fa-solid fa-envelope ml-1"></i></button>
                                    </div>
                                </div>
                            </div>
                        </div>

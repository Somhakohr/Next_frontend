...
function OrganisationCandidateProfileView(props) {
    ...
    function verifyIntPopup() {
        return title.length > 0 && round.length > 0 && date.length > 0 && stime.length > 0 && etime.length > 0
    }
    ...
    async function loadApplicantData(id) {
        await axiosInstanceAuth2.get('/job/single/applicant/'+id+'/').then(async (res)=>{
    ...
    async function loadInterview(id,erfid) {
        await axiosInstanceAuth2.get('/job/interviews/'+id+'/'+erfid+'/').then(async (res)=>{
    ...
    function goback() {
        updateParam1('')
        router.push('/organisation/applicants')
    }
    ...
    async function loadLang() {
        await axiosInstanceAuth2.get('/candidate/listlang/'+crefid+'/').then(async (res)=>{
            setLang(res.data)
        }).catch((err)=>{
    ...
    async function loadLink() {
        await axiosInstanceAuth2.get('/candidate/listlink/'+crefid+'/').then(async (res)=>{
            setLink(res.data)
        }).catch((err)=>{
            if(err.message != "Request failed with status code 401"){
    ...
    async function loadReume() {
        await axiosInstanceAuth2.get('/candidate/listresume/'+crefid+'/').then(async (res)=>{
            setResume(res.data)
        }).catch((err)=>{
            if(err.message != "Request failed with status code 401"){
    ...
    async function loadSkill() {
        await axiosInstanceAuth2.get('/candidate/listskill/'+crefid+'/').then(async (res)=>{
            setSkill(res.data)
        }).catch((err)=>{
            if(err.message != "Request failed with status code 401"){
    ...
    async function loadCertification() {
        await axiosInstanceAuth2.get('/candidate/listcertificate/'+crefid+'/').then(async (res)=>{
            setCert(res.data)
        }).catch((err)=>{
            if(err.message != "Request failed with status code 401"){
    ...
    async function loadEducation() {
        await axiosInstanceAuth2.get('/candidate/listeducation/'+crefid+'/').then(async (res)=>{
            setEdu(res.data)
        }).catch((err)=>{
            if(err.message != "Request failed with status code 401"){
    ...
    async function loadExperience() {
        await axiosInstanceAuth2.get('/candidate/listexperience/'+crefid+'/').then(async (res)=>{
            setExp(res.data)
        }).catch((err)=>{
            if(err.message != "Request failed with status code 401"){
    ...
    async function loadAchieve() {
        await axiosInstanceAuth2.get('/candidate/listachievement/'+crefid+'/').then(async (res)=>{
            setAchieve(res.data)
        }).catch((err)=>{
            if(err.message != "Request failed with status code 401"){
    ...
    async function updateStatus(status,arefid,refid) {
        await axiosInstanceAuth2.post('/job/'+status+'/applicant/'+arefid+'/'+refid+'/').then(async (res)=>{
            loadApplicantData(param1)
    ...
    async function addInterview() {
        ...
        await axiosInstanceAuth2.post('/job/create/interview/'+refid+'/'+crefid+'/',formdata).then(async (res)=>{
            loadInterview(refid,crefid)
    ...
    async function updateIStatus(status,pk,refid,arefid) {
        await axiosInstanceAuth2.post('/job/'+status+'/interview/'+pk+'/'+refid+'/').then(async (res)=>{
            loadInterview(refid,crefid)
            toastcomp("Interview Status Updated",'success')
            if(status == "passed"){updateStatus('hire',arefid,refid)}
            if(status == "reject"){updateStatus('reject',arefid,refid)}
            if(status == "hold"){updateStatus('hold',arefid,refid)}
        ...
        <>
            ...
            {applicantData.map((data, i) => (
                <div className="container" key={i}>
                    <button type="button" onClick={(e)=>goback()} className="mb-2 rounded-full bg-black text-white p-4 mr-4 w-[25px] h-[25px] flex items-center justify-center">
                            ...
                            <div className="bg-white shadow-normal border border-teal-400 rounded-[25px] p-6">
                                ...
                                <ul className="flex flex-wrap mx-[-10px] text-[#646464]">
                                    ...
                                    <li className="w-[50%] px-[10px]">
                                        <button type="button" className="w-full rounded-full py-1.5 px-6 mt-3 text-left bg-[#6D27F9] text-white" disabled>
                                    ...
                                    :
                                    <li className="w-[50%] px-[10px]">
                                        <button type="button" className="w-full rounded-full py-1.5 px-6 mt-3 text-left hover:bg-[#6D27F9] hover:text-white" onClick={(e)=>updateStatus('shortlist',data.arefid,data.job.refid)}>
                                            <i className="fa-solid fa-thumbs-up mr-2"></i>
                                            Shortlist
                                        </button>
                                    </li>
                                    ...
                                    {data.status == "Hold" ? 
                                    <li className="w-[50%] px-[10px]">
                                        <button type="button" className="w-full rounded-full py-1.5 px-6 mt-3 text-left bg-[#FEF401] text-black" disabled>
                                    ...
                                    :
                                    <li className="w-[50%] px-[10px]">
                                        <button type="button" className="w-full rounded-full py-1.5 px-6 mt-3 text-left hover:bg-[#FEF401] hover:text-black" onClick={(e)=>updateStatus('hold',data.arefid,data.job.refid)}>
                                            <i className="fa-solid fa-circle-pause mr-2"></i>
                                            On Hold
                                        </button>
                                    </li>
                                    ...
                                    {data.status == "Hired" ? 
                                    <li className="w-[50%] px-[10px]">
                                        <button type="button" className="w-full rounded-full py-1.5 px-6 mt-3 text-left bg-[#58E780] text-white" disabled>
                                    ...
                                    :
                                    <li className="w-[50%] px-[10px]">
                                        <button type="button" className="w-full rounded-full py-1.5 px-6 mt-3 text-left hover:bg-[#58E780] hover:text-white" onClick={(e)=>updateStatus('hire',data.arefid,data.job.refid)}>
                                            <i className="fa-solid fa-user mr-2"></i>
                                            Hire
                                        </button>
                                    </li>
                                    ...
                                    <li className="w-[50%] px-[10px]">
                                        <button type="button" className="w-full rounded-full py-1.5 px-6 mt-3 text-left bg-[#FF5E5E] text-white" disabled>
                                    ...
                                    :
                                    <li className="w-[50%] px-[10px]">
                                        <button type="button" className="w-full rounded-full py-1.5 px-6 mt-3 text-left hover:bg-[#FF5E5E] hover:text-white" onClick={(e)=>updateStatus('reject',data.arefid,data.job.refid)}>
                                            <i className="fa-solid fa-thumbs-down mr-2"></i>
                                            Reject
                                        </button>
                                    </li>
                                    ...
                                    {interview.map((data2, i) => (
                                        ...
                                        :
                                        <li className="w-[50%] px-[10px]">
                                            <button type="button" className="w-full rounded-full py-1.5 px-6 mt-3 text-left hover:bg-[#FEF401] hover:text-black" onClick={(e)=>updateIStatus('hold',data2.id,data.job.refid,data.arefid)}>
                                                <i className="fa-solid fa-circle-pause mr-2"></i>
                                                On Hold
                                            </button>
                                        </li>
                                        ...
                                        :
                                        <li className="w-[50%] px-[10px]">
                                            <button type="button" className="w-full rounded-full py-1.5 px-6 mt-3 text-left hover:bg-[#58E780] hover:text-white" onClick={(e)=>updateIStatus('passed',data2.id,data.job.refid,data.arefid)}>
                                                <i className="fa-solid fa-user mr-2"></i>
                                                Hire
                                            </button>
                                        </li>
                                        ...
                                        :
                                        <li className="w-[50%] px-[10px]">
                                            <button type="button" className="w-full rounded-full py-1.5 px-6 mt-3 text-left hover:bg-[#FF5E5E] hover:text-white" onClick={(e)=>updateIStatus('reject',data2.id,data.job.refid,data.arefid)}>
                                                <i className="fa-solid fa-thumbs-down mr-2"></i>
                                                Reject
                                            </button>
                                        </li>
                                ...
                                {data.status == "Hired" || data.status == "Rejected" || data.status == "Hold" || data.status == "Shortlisted" ? <>
                                {interview.length <= 0 ?<>
                                {data.status == "Shortlisted" ? 
                                <>
                                <div className="text-right border-t border-slate-300 pt-3 mt-3 mb-6">
                                    <button type="button" className="disabled:opacity-30 disabled:cursor-normal bg-gradient-to-r from-[#6D27F9] to-[#9F09FB] text-white font-semibold text-sm rounded-full py-1.5 px-4 md:min-w-[150px] transition-all hover:from-[#391188] hover:to-[#391188]" onClick={() => addRoundPopupOpen(true)}>+ Add Interview</button>
                                </div>
                                </>
                                 : 
                                 <>
                                 <div className="text-right border-t border-slate-300 pt-3 mt-3 mb-6">
                                     <button type="button" className="disabled:opacity-30 disabled:cursor-normal bg-gradient-to-r from-[#6D27F9] to-[#9F09FB] text-white font-semibold text-sm rounded-full py-1.5 px-4 md:min-w-[150px] transition-all hover:from-[#391188] hover:to-[#391188]" disabled>+ Add Interview</button>
                                 </div>
                                 </>}</>
                                ...
                                <div className="max-w-[350px] ml-auto">
                                    {interview.length <= 0 ? 
                                     ...
                                     <div className="relative z-[2]">
                                         {data.status == "Hired" && <span className="bg-white mb-1 inline-block py-1.5 px-4 text-sm border border-[#58E780]  text-center rounded-full min-w-[115px]">Hired</span>}
                                         {data.status == "Rejected" && <span className="bg-white mb-1 inline-block py-1.5 px-4 text-sm border border-[#FF5E5E]  text-center rounded-full min-w-[115px]">Rejected</span>}
                                         {data.status == "Hold" && <span className="bg-white mb-1 inline-block py-1.5 px-4 text-sm border border-[#FEF401]  text-center rounded-full min-w-[115px]">On Hold</span>}
                                         {data.status == "Shortlisted" && <span className="bg-white mb-1 inline-block py-1.5 px-4 text-sm border border-[#6D27F9]  text-center rounded-full min-w-[115px]">Shortlisted</span>}
                                     </div>
                                    ...
                                    {interview.map((data2, i) => (
                                        ...
                                        <div className="relative z-[2]">
                                            ...
                                            {data2.status == "Reject" && <span className="bg-white mb-1 inline-block py-1.5 px-4 text-sm border border-[#FF5E5E] text-center rounded-full min-w-[115px]">
                                                {data2.title}
                                            </span>}
                                            {data2.status == "Hold" && <span className="bg-white mb-1 inline-block py-1.5 px-4 text-sm border border-[#Hold] text-center rounded-full min-w-[115px]">
                                                {data2.title}
                                            </span>}
                                            {data2.status == null && <span className="bg-white mb-1 inline-block py-1.5 px-4 text-sm border text-center rounded-full min-w-[115px]">
                                                {data2.title}
                                            </span>}
                                        </div>
                                        <p className="text-[#646464] text-[12px]">{data2.interview_date} {data2.interview_start_time}</p>
                                    </div>
                                    ))}
                                    </>
                                    }
                            ...
                            <div className="p-8">
                                ...
                                <aside>
                                    <div className="mb-6">
                                        ...
                                        <input type="text" id="interViewTitle" className="w-full rounded-full border-slate-300" value={title} onChange={(e)=>settitle(e.target.value)}/>
                                    </div>
                                    <div className="flex flex-wrap justify-between">
                                        <div className="w-full lg:w-[47%] mb-6">
                                            ...
                                            <input id="interViewRound" type="text" className="w-full rounded-full border-slate-300"  value={round} onChange={(e)=>setround(e.target.value)}/>
                                        </div>
                                        <div className="w-full lg:w-[47%] mb-6">
                                            <label htmlFor="interViewDate" className="font-medium mb-2 leading-none inline-block">Interview Date</label>
                                            <input id="interViewDate" type="date" className="w-full rounded-full border-slate-300"  value={date} onChange={(e)=>setdate(e.target.value)}/>
                                        </div>
                                    </div>
                                    <div className="flex flex-wrap justify-between">
                                        <div className="w-full lg:w-[47%] mb-6">
                                            <label htmlFor="interViewStartTime" className="font-medium mb-2 leading-none inline-block">Interview Start Time</label>
                                            <input id="interViewStartTime" type="time" className="w-full rounded-full border-slate-300"  value={stime} onChange={(e)=>setstime(e.target.value)}/>
                                        </div>
                                        <div className="w-full lg:w-[47%] mb-6">
                                            <label htmlFor="interViewEndTime" className="font-medium mb-2 leading-none inline-block">Interview End Time</label>
                                            <input id="interViewEndTime" type="time" className="w-full rounded-full border-slate-300" value={etime} onChange={(e)=>setetime(e.target.value)} />
                                        </div>
                                    </div>
                                    <div className="text-center">
                                        <button type="button" className="disabled:opacity-30 disabled:cursor-normal bg-gradient-to-r from-[#6D27F9] to-[#9F09FB] text-white font-bold rounded-full py-2.5 px-6 md:min-w-[150px] transition-all hover:from-[#391188] hover:to-[#391188]" onClick={(e)=>addInterview()} disabled={!verifyIntPopup()}>Submit</button>
                                    </div>
                                </aside>
                            </div>
                        </Dialog.Panel>
